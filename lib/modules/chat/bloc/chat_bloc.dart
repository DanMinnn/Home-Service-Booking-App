...
import 'package:home_service_tasker/modules/chat/repo/chat_repo.dart';
...
class ChatBloc extends Bloc<ChatEvent, ChatState> {
  ...
  //cache room and message
  List<ChatRoomModel> roomCache = [];
  List<ChatMessageModel> messagesCache = [];
  ...
  Future<void> _onChatRoomsLoaded(
    ...
    try {
      emit(ChatLoading());
      final rooms = await _chatRepo.getChatRooms(event.taskerId);
      roomCache = rooms;
  ...
  Future<void> _onChatRoomSelected(
    ...
    // If we have cached messages, emit them first
  ...
  Future<void> _onChatMessageSent(
    ...
    try {
      ...
      final tempMessage = ChatMessageModel(
        ...
        roomId: event.roomId,
        senderId: _chatService.taskerId!,
      ...
      // The real message will come back via WebSocket and be handled by ChatMessageReceived
    } catch (e) {
